...
echo "> docker run -d --name ${server_name} --env-file application-secret.properties -p 8080:8080 ${docker_repo}/${docker_image}"
docker run -d --name ${server_name} --env-file application-secret.properties -p 8080:8080 ${docker_repo}/${docker_image}